...
export default class Multiview extends React.Component {
    ...
    componentDidMount() {
        this.props.dispatch(allActionDucer(SPORTSBOOK_ANY,{activeView: 'Multiview',loadSports:true}))
    ...
    render() {
      ...
      const{ data, loadCompetition, loadSports, loadMarket, activeGame, activeSport, competitionData, activeView, competitionName,
          activeCompetition, activeRegion, competitionRegion,multiviewGames, betSelections, activeGameSuspended, sportAlias, oddType 
      } = this.props.sportsbook,{addEventToSelection, loadMarkets, loadGames,unsubscribe,retrieve,validate,getBetslipFreebets,sendRequest,subscribeToSelection}=this.props
      const sport = data ? data.sport : {}
      let newdata = [], marketGroups = [], searchResult = [], multiviewSelectedGames = {}
