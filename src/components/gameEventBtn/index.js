...
export default class GameEventBtn extends React.Component {
    ...
    render() {
      const {
        props: { evnt, game, evtmarket, betSelections, oddType ,size,competition,sport,region}
      } = this
      return (
        <div ref={(el) => { this.priceBtn = el }} onClick={(e) =>  {e.stopPropagation();this.selectEvent(game.is_blocked,game, evtmarket, evnt,competition,sport,region)}  } className={`${size? size:''} ${evnt.name.toLowerCase()} ${betSelections[evtmarket.id] && betSelections[evtmarket.id].eventId == evnt.id ? 'selected-event' : ''} ${evnt.initialPrice ? (evnt.initialPrice > evnt.price) ? 'coeficiente-change-down' : (evnt.initialPrice < evnt.price) ? 'coeficiente-change-up' : '' : ''}`}>
