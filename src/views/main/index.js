...
handlePageVisibilityChange = (e) => {
  if(document.visibilityState === 'hidden'){
    ...
    if(this.state.inactiveTimedout){
      ...
      if(this.socket.getReadyState() === WebSocket.CLOSED){this.connectSocket()}
    }
  }
}
headerScroll(e) {
...
autoGetBalance(){
  ...
  handleSportData(data) {
    ...
    subscriptions[data.subid] = { subid: data.subid, callback: (data) => this.handleSportUpdate(data, 'data'), subStateVarName: 'sportsubid',request:this.rids[data.rid].request }
    ...
    if (viewmode === 0) {
      ...
      dispatch(allActionDucer(SPORTSBOOK_ANY, { sportsubid: data.subid, data: data.data, subscriptions: subscriptions, loadSports: false }))
    } else if (viewmode === 2) {
      dispatch(allActionDucer(SPORTSBOOK_ANY, { sportsubid: data.subid, data: data.data, subscriptions: subscriptions, loadSports: false }))
    } else if (viewmode === 3) {
      sport ? dispatch(allActionDucer(SPORTSBOOK_ANY, {sportsubid: data.subid, data: data.data, subscriptions: subscriptions, loadSports: false }))
        :
  ...
  handleMarketData(data) {
    ...
    this.props.dispatch(allActionDucer(SPORTSBOOK_ANY, { inviewMarketsubid: data.subid, marketData: marketData,activeGame:activeGame, loadMarket: false }))
  }
  ...
  handleLiveNow(data) {
    ...
    subscriptions[data.subid] = { subid: data.subid, callback: (data) => this.handleSportUpdate(data, 'liveNowData'), subStateVarName: 'liveNowSubId',request:this.rids[data.rid].request  }
    this.subscriptions = subscriptions
  ...
  handleUpcoming(data) {
    ...
    subscriptions[data.subid] = { subid: data.subid, callback: (data) => this.handleSportUpdate(data, 'upcomingData'), subStateVarName: 'upcomingSubId' ,request:this.rids[data.rid].request }
    this.subscriptions = subscriptions
  ...
  handleGameUpdate(data) {
    ...
    data.game && Object.keys(data.game).forEach((gameID) => {
      if (data.game[gameID]) {
  ...
  handleMarketUpdate(data) {
    ...
    let  marketData = {...this.props.sportsbook.marketData},activeGame={...this.props.sportsbook.activeGame},dataclone = {...data};
    ...
    if (null !== activeGame) {
      ...
      this.props.dispatch(allActionDucer(SPORTSBOOK_ANY, {activeGame:activeGame}))
      }
    data.hasOwnProperty('market') && Object.keys(data.market).forEach((marketID) => {
  ...
  betSuccess(data) {
    ...
    if (betSlipMode === 2) {
      ...
      this.betPlacedTimeout = setTimeout(() => {
        dispatch(allActionDucer(SPORTSBOOK_ANY, { betPlaced: false,betSelections: {} }));
        dataStorage('betSlip', {}, 2)
      }, 5000);
      this.props.dispatch(allActionDucer(SPORTSBOOK_ANY, {betMode: 1, betStake: 0 }))
      selectionSub && this.unsubscribe(selectionSub) && dispatch(allActionDucer(SPORTSBOOK_ANY, { selectionSub: null }))
      const userId = getCookie('id'), authToken = getCookie('AuthToken'),mobile = getCookie('mobile'),$time = moment().format('YYYY-MM-DD H:mm:ss'),hash=calcMD5(`AuthToken${authToken}uid${userId}mobile${mobile}time${$time}${this.props.appState.$publicKey}`);
  ...
  loadMultiviewLiveGames() {
    ...
    this.rids[7].request = {
      ...
      "params": {
        "source": "betting",
        "what": { sport: ["id", "name", "alias","order"], region: ["id", "name", "alias", "order"], competition: ["id", "order", "name"], game: [[]], market: [], event: [] },
  ...
  loadPrematchGames() {
    ...
    this.rids[7].request = {
      ...
      params: {
        source: "betting",
        what: { sport: ["id", "name", "alias","order"], region: ["id", "name", "alias", "order"], competition: ["id", "order", "name"], game: ["@count"] },
      ...
      <div className="main-container">
        ...
        <div className="main-body">
          <div className="body-contents" id="body-contents">
            ...
            <div className="dynamic-content">
              ...
              <Switch>
                ...
                <Route path="/sports" render={(props) => <SportsBook {...props} getEvents={this.getEvents}
                  addEventToSelection={this.addEventToSelection} loadGames={this.loadGames}
                  loadMarkets={this.loadMarkets} sendRequest={this.sendRequest} unsubscribe={this.unsubscribe} handleBetResponse={this.handleBetResponse}
                  subscribeToSelection={this.subscribeToSelection.bind(this)}
                  retrieve={this.getEvents} getBetslipFreebets={this.getBetslipFreebets.bind(this)}
                  bulkUnsubscribe={this.bulkUnsubscribe} loadLiveGames={this.loadLiveGames} loadPrematchGames={this.loadPrematchGames}
                  loadLiveOverviewGames={this.loadLiveOverviewGames.bind(this)} loadMultiviewLiveGames={this.loadMultiviewLiveGames.bind(this)} />} />
                <Route path="/virtual">
                  <Virtual bulkUnsubscribe={this.bulkUnsubscribe} lang={this.lang} socket={this.socket} reconnect={this.connectSocket.bind(this)}/>
                </Route>
                <Route path="/roulette">
                  <Roulette bulkUnsubscribe={this.bulkUnsubscribe} lang={this.lang}/>
                </Route>
                <Route path="/slot-games"
                  render={(props)=><SlotGames {...props}
                  addEventToSelection={this.addEventToSelection} loadGames={this.loadGames}
                  loadMarkets={this.loadMarkets} sendRequest={this.sendRequest} unsubscribe={this.unsubscribe}
                  subscribeToSelection={this.subscribeToSelection.bind(this)}
                  retrieve={this.getEvents} getBetslipFreebets={this.getBetslipFreebets.bind(this)}
                  loadData = {this.loadPrematchGames} bulkUnsubscribe={this.bulkUnsubscribe}
                  />}
                />
                <Route path="/news"
            ...
            <Footer activeView={this.props.sportsbook.activeView}  appState={this.props.appState} dispatch={this.props.dispatch}/>
          </div>
        </div>
      </div>
    </Router>
